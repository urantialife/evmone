...
template <typename FnT, evmc_opcode Op>
[[gnu::always_inline]] inline bool invoke(
    FnT fn, ExecutionState& state, const uint8_t*& code_it) noexcept
{
    if constexpr (std::is_invocable_v<FnT, ExecutionState&, size_t>)
    {
        fn(state, )
    }
...
#define INSTR_IMPL(OPCODE)                                                                   \
    case OPCODE:                                                                             \
    {                                                                                        \
        if constexpr (std::is_invocable_v<decltype(op2fn::OPCODE), ExecutionState&, size_t>) \
        {}                                                                                   \
        else if constexpr (instr::traits[OPCODE].terminator)                                 \
        {                                                                                    \
            state.status = std::invoke(op2fn::OPCODE, state);                                \
            goto exit;                                                                       \
        }                                                                                    \
        DISPATCH();                                                                          \
    }
...
#define REST                                                                                       \
                                                                                                   \
    if (const auto r = instr::implementations[OPCODE](state, static_cast<size_t>(code_it - code)); \
